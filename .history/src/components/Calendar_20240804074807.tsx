  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push(
        <div
          key={i}
          className={`p-2 border cursor-pointer hover:bg-gray-100 ${noteCount > 0 ? 'bg-blue-100' : ''}`}
          onClick={() => onDateClick(date)}
        >
          <span className="font-bold">{i}</span>
          {noteCount > 0 && (
            <div className="w-5 h-5 bg-blue-500 rounded-full ml-1 inline-flex items-center justify-center text-white text-xs">
              {noteCount}
            </div>
          )}
        </div>
